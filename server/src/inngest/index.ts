...
import { IBooking, IShow, IUser, IMovie } from "../types/index.js";
...
const releaseSeatsAndDeleteBooking = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("check-payment-status", async () => {
      ...
      const booking = await Booking.findById(bookingId) as IBooking | null;

      // If payment is not made, release seats and delete booking
      if (booking && !booking.isPaid) {
        const show = await Show.findById(booking.show) as IShow | null;
        if (show) {
...
const sendBookingConfirmationEmail = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const booking = await Booking.findById(bookingId)
      ...
      .populate("user") as (IBooking & { user: IUser; show: IShow & { movie: IMovie } }) | null;
  ...
  async ({ step }) => {
    ...
    const results = await step.run("send-all-reminders", async () => {
      return await Promise.allSettled(
        reminderTasks.map((task: ReminderTask) =>
          sendEmail({
            to: task.userEmail,
        ...
        <p>
          is scheduled for <strong>${new Date(task.showDateTime).toLocaleDateString(
            "en-US",
            { timeZone: "Africa/Kigali" }
          )}</strong> at
          <strong>${new Date(task.showDateTime).toLocaleTimeString("en-US", {
            timeZone: "Africa/Kigali",
          })}</strong>.
        </p>
